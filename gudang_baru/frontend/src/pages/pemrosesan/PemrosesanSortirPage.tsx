              ...
              {item.detailKomoditas && Array.isArray(item.detailKomoditas) && (
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 grid grid-cols-1 gap-2">
                  {item.detailKomoditas.map((dk: any, idx: number) => {
                    ...
                    else if (dk.kemasan === 'kombinasi') targetKemasanText = `Kombinasi (2.5Kg: ${dk.kemasanKombinasiBesar} pack, sisanya 1Kg)`;
                    ...
                    <div key={idx} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                      <div className="bg-emerald-50/50 px-4 py-2 border-b border-emerald-100 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                          <button 
                            onClick={() => {
                              const avgUpah = (totalPekerjaKg * 1500) / modal.detailKomoditas!.length;
                              setHppTenagaKerja(prev => ({...prev, [idx]: avgUpah.toString()}));
                            }}
                            className="text-[10px] bg-emerald-100 hover:bg-emerald-200 text-emerald-700 px-2 py-1 rounded font-bold transition-colors"
                          >
                            Ambil Tenaga Kerja (Rata-rata)
                          </button>
                        </div>
                      </div>
